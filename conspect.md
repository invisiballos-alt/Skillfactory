...
```
# PostgreSQL
создать нову БД можно через psql - терминал, либо через pgAdmin - графический интерфейс. 
PostgreSQL запускает сервер на ПК. На сервере находится новая БД.
## Создание БД на сервере PostgreSQL по умолчанию (сервер PostgreSQL 18)
c помощью psql: 
```

```
Чтобы подружить Django из WSL с PostgreSQL на Windows, вам нужно сделать два простых действия: узнать IP-адрес вашей Windows-системы внутри WSL и разрешить PostgreSQL принимать сетевые подключения: 
1. Находим IP-адрес Windows для WSL
```bash
ip route show | grep default | awk '{print $3}'
```

2. Проверяем подключение по порту с помощью netcat:
```bash
nc -zv 172.31.96.1 5433
```



# Visual Studio Code (Vscode):
- настройка сниппета. Для markdown (.md):
1. В VS Code нажмите комбинацию клавиш Ctrl + Shift + P 
2. Preferences: Configure User Snippets
3. В выпадающем списке введите markdown и выберите пункт markdown.json
...
## Объекты. ООП. Objects. 
### Классы Classes
- Класс (Class) — это чертёж, шаблон. Сам по себе класс не хранит данные, только описывает структуру, по которой будут создаваться объекты (экземпляры класса).
